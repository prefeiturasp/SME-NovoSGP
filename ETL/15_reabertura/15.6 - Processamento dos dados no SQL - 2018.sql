...
CASE 
	WHEN CHARINDEX('EJA',c.cal_descricao) = 0 THEN 7 -- não é EJA
	 	 ELSE CASE when tpc_id = 1 then 8
		           when tpc_id = 2 then 9
			  END
END												as tipo
from ACA_Evento 					        ev (NOLOCK)
inner join ACA_CalendarioEvento				ce (NOLOCK)
            on ev.evt_id = ce.evt_id
inner join ACA_CalendarioAnual				c (NOLOCK)
            on c.cal_id = ce.cal_id
inner join ESC_Escola						es (NOLOCK)
...
where    tev_id = 1
		 ...
		 and (evt_nome not like ('%retifica%') AND
	      ...
	      and((tpc_id = 1 and (evt_dataInicio between '2018-04-01' and '2018-05-30'))
		  or (tpc_id = 2 and (evt_dataInicio between '2018-06-01' and '2018-07-30'))
		  or (tpc_id = 3 and (evt_dataInicio between '2018-09-01' and '2018-10-30'))
		  or (tpc_id = 4 and (evt_dataInicio between '2018-11-01' and '2018-12-22')))
