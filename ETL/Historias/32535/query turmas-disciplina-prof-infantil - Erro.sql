...
INSERT INTO #ResponsaveisUe
 select distinct ue.cd_unidade_educacao, servsub.NomeServidor, servsub.CodigoRf from v_cadastro_unidade_educacao dre
                        ...
                        AND etapa_ensino.cd_etapa_ensino IN ( 1, 2, 3, 7, 11, 4, 5, 12, 13, 6, 7, 8, 9, 17, 14 ) 
...
select distinct 
										...
										MIN(coalesce(serv.cd_registro_funcional, resp.CodigoRf, '')) as ProfessorRF,
										MIN(coalesce(serv.nm_pessoa, resp.NomeServidor, '')) as NomeProfessor,
										ue.cd_unidade_educacao,
										[dbo].[proc_gerar_email_funcionario](MIN(coalesce(serv.nm_pessoa, resp.NomeServidor, '')), MIN(coalesce(serv.cd_registro_funcional, resp.CodigoRf, ''))) EmailProfessor
                    ...
                    where  te.st_turma_escola in ('O', 'A', 'C')
					 ...
					 and   te.cd_turma_escola = '2335246'
					group by 
					ue.cd_unidade_educacao,
				 ...
				 inner join tmp_tbl_gc_erro_sa erro on erro.TurmaId = resultado.TurmaId and erro.ComponenteCurricularId = resultado.ComponenteCurricularId
