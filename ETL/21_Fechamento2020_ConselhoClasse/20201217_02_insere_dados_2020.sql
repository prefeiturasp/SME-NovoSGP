...
BEGIN
	...
	loop
		...
		loop
			...
			for fechamentoNota in select distinct fn.disciplina_id as disciplina, fn.conceito_id as conceito, fn.nota as nota from fechamento_turma_disciplina ftd 
				...
				where ftd.fechamento_turma_id = fechamentoTurma.id
				and fa.aluno_codigo = fechamentoAluno.codigo
				and cc.permite_lancamento_nota
			loop
				insert into conselho_classe_nota (conselho_classe_aluno_id, justificativa, componente_curricular_codigo, nota, conceito_id, criado_em, criado_por, criado_rf) 
					values(conselhoClasseAlunoId,'', fechamentoNota.disciplina, fechamentoNota.nota, fechamentoNota.conceito, now(), 'SISTEMA', '0');
			end loop;				
		end loop;
	commit;
	end loop;
