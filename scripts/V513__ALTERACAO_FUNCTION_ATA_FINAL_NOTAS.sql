CREATE OR REPLACE FUNCTION public.f_ata_final_obter_notas_turmas_alunos(p_ano_letivo integer DEFAULT 2021, p_modalidade_codigo integer DEFAULT 0, p_semestre integer DEFAULT 0, p_tipos_turma integer[] DEFAULT NULL::integer[], p_alunos_codigos character varying[] DEFAULT NULL::character varying(15)[])
...
union all
 ...
 on
	...
	where
		...
		and (p_modalidade_codigo = 0 or (p_modalidade_codigo <> 0 and t.modalidade_codigo = p_modalidade_codigo))
		and (p_semestre = 0 or (p_semestre <> 0 and t.semestre = p_semestre))
		and (p_tipos_turma is null or (array_length(p_tipos_turma, 1) > 0 and t.tipo_turma = any(p_tipos_turma)))) x order by x.NotaId desc
$function$
;
