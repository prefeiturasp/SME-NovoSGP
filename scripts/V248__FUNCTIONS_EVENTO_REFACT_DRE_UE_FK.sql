...
AS $function$
	...
	where not e.excluido 
		...
		and ((p_dre_id is null and ((e.dre_id is null and e.ue_id is null) or e.dre_id in (select dre_id from f_abrangencia_dres(p_login, p_perfil_id, p_historico)))) or (p_dre_id is not null and ((e.dre_id is null and e.ue_id is null) or e.dre_id = p_dre_id)))
		and ((p_ue_id is null and (e.ue_id is null or e.ue_id in (select ue_id from f_abrangencia_ues(p_login, p_perfil_id, p_historico)))) or (p_ue_id is not null and (e.ue_id is null or e.ue_id = p_ue_id)))
...
CREATE OR REPLACE FUNCTION public.f_eventos_por_rf_criador(p_login character varying, p_tipo_calendario_id bigint, p_dre_id bigint DEFAULT NULL, p_ue_id bigint DEFAULT NULL, p_data_inicio date DEFAULT NULL::date, p_data_fim date DEFAULT NULL::date)
 RETURNS SETOF v_estrutura_eventos
 LANGUAGE sql
AS $function$
	select e.id,
