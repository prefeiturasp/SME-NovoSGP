...
CREATE OR REPLACE FUNCTION public.f_abrangencia_turmas_tipos_lista_ues(
    ...
    p_ue_codigos character varying[] DEFAULT NULL::character varying[],
...
RETURNS TABLE(
    ...
    turma_id bigint,
    tipoturma integer,
    nome_filtro character varying,
    ue_codigo character varying
)
LANGUAGE sql
AS $function$
...
where a.login = p_login
    ...
    and (p_turma_semestre = 0 or (p_turma_semestre <> 0 and act.turma_semestre = p_turma_semestre))
    and (p_ue_codigos is null or act.ue_codigo = ANY(p_ue_codigos))
    and (p_ano_letivo = 0 or (p_ano_letivo <> 0 and act.turma_ano_letivo = p_ano_letivo))
    and (p_tipos_turma is null or (array_length(p_tipos_turma, 1) > 0 and act.tipo_turma = ANY(p_tipos_turma)))
    and (
        p_anos_desconsiderar_turma_infantil is null
        or act.modalidade_codigo <> 1
        or (array_length(p_anos_desconsiderar_turma_infantil, 1) > 0 and act.modalidade_codigo = 1 and not act.turma_ano = ANY(p_anos_desconsiderar_turma_infantil))
    )
