...
select distinct e.id,
                e.data_inicio + i as data_evento,
                e.data_inicio,
                e.data_fim,
                e.nome,
                et.descricao as tipoEvento,                
                coalesce(ad.abreviacao, 'Todas') as DreNome,
                coalesce(au.nome, 'Todas') as DreNome,         
                au.tipoescola as ueTipo,
                e.descricao
from evento e
 inner join generate_series(0, (e.data_fim - e.data_inicio)) as i on 1=1
...
where p_dataReferencia = data_evento
