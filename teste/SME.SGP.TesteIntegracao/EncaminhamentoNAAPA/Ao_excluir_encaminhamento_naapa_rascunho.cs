...
{
    public class Ao_excluir_encaminhamento_naapa_rascunho : EncaminhamentoNAAPATesteBase
    {
        ...
        [Fact(DisplayName = "Encaminhamento NAPA - Usuário não gestor (Ex.: Perfil Professor) e não Criador do encaminhamento não pode excluir encaminhamento")]
        public async Task Ao_excluir_encaminhamento_rascunho_outro_usuario_nao_gestor()
        {
            var filtroNAAPA = new FiltroNAAPADto()
            {
            ...
            var exceptionNAAPA = await Assert.ThrowsAsync<NegocioException>(() => excluirEncaminhamentoNaapaUseCase.Executar(1));
            Assert.Equal(MensagemNegocioEncaminhamentoNAAPA.ENCAMINHAMENTO_NAO_PODE_SER_EXCLUIDO_PELO_USUARIO_LOGADO, exceptionNAAPA.Message);

            encaminhamentoNAAPA = ObterTodos<Dominio.EncaminhamentoNAAPA>();
            encaminhamentoNAAPA.ShouldNotBeNull();
            encaminhamentoNAAPA.Where(encaminhamento => !encaminhamento.Excluido).Count().ShouldBe(1);

            encaminhamentoNAAPASecao = ObterTodos<EncaminhamentoNAAPASecao>();
            encaminhamentoNAAPASecao.ShouldNotBeNull();
            encaminhamentoNAAPASecao.FirstOrDefault().SecaoEncaminhamentoNAAPAId.ShouldBe(1);
            encaminhamentoNAAPASecao.Where(encaminhamentoSecao => !encaminhamentoSecao.Excluido).Count().ShouldBe(1);
