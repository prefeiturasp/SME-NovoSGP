...
using Xunit;
using TurmaDominio = SME.SGP.Dominio.Turma;
...
namespace SME.SGP.Aplicacao.Teste.Queries.EOL.Aluno.Turmas.ObterTurmasComMatriculaValidasParaValidarConselho
{
    public class ObterTurmasComMatriculaValidasParaValidarConselhoQueryHandlerTeste
    {
        ...
        public async Task NaoDeveConsiderarMatriculaAtivaIniciadaAposPeriodoEscolar()
        {
            ...
            var turma = new TurmaDominio { CodigoTurma = "1", TipoTurma = TipoTurma.Regular };
            ...
            ConfigurarMocks(turma, matriculas);

            var query = CriarQuery(turma, anoAtual);
        ...
        [Fact(DisplayName = "ObterTurmasComMatriculaValidasParaValidarConselhoQuery - Não considerar vínculo indevido")]
        public async Task NaoDeveConsiderarVinculoIndevido()
        {
            var anoAtual = DateTime.Now.Year;
            var turma = new TurmaDominio { CodigoTurma = "1", TipoTurma = TipoTurma.Regular };
            var matriculas = new List<AlunoPorTurmaResposta>
            {
                new()
                {
                    CodigoAluno = "1",
                    DataMatricula = new DateTime(anoAtual, 2, 10),
                    DataSituacao = new DateTime(anoAtual, 5, 10),
        ...
        private void ConfigurarMocks(TurmaDominio turma, IEnumerable<AlunoPorTurmaResposta> matriculas)
        {
        ...
        private static ObterTurmasComMatriculaValidasParaValidarConselhoQuery CriarQuery(TurmaDominio turma, int anoAtual)
            => new("1", new[] { turma.CodigoTurma }, new DateTime(anoAtual, 2, 4), new DateTime(anoAtual, 4, 30));
    }
}
