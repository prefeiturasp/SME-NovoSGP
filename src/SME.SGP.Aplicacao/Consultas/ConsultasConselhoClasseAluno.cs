...
namespace SME.SGP.Aplicacao
{
    public class ConsultasConselhoClasseAluno : IConsultasConselhoClasseAluno
    {
            ...
            string alunoCodigo, string codigoTurma, int bimestre)
        {
            ...
            var frequenciaAluno = await mediator.Send(new ObterFrequenciasAlunoComponentePorTurmasQuery(alunoCodigo, new[] { turma.CodigoTurma }, tipoCalendarioId, bimestre));
            ...
            var registrosFrequencia = await mediator.Send(
                new ObterFrequenciasRegistradasPorTurmasComponentesCurricularesQuery(alunoCodigo, new[]
                        { turma.CodigoTurma },
                    disciplinas.Select(d =>
                        !d.TerritorioSaber
                            ? d.CodigoComponenteCurricular.ToString()
                            : d.CodigoComponenteTerritorioSaber.ToString()).ToArray(),
                    periodoEscolar?.Id));
            ...
            foreach (var grupoDisiplinasMatriz in gruposMatrizes.OrderBy(k => k.Key.Nome))
            {
                ...
                foreach (var componenteCurricular in grupoDisiplinasMatriz.Where(x => !x.LancaNota))
                {
                    ...
                    {
                        var totalAulasNaoPermitemFrequencia = await mediator.Send(new ObterTotalAulasSemFrequenciaPorTurmaQuery(componenteCurricular.CodigoComponenteCurricular.ToString(), codigoTurma));
            ...
            long fechamentoTurmaId, string alunoCodigo, string codigoTurma, int bimestre, bool consideraHistorico = false)
        {
            ...
            if (turmasComMatriculasValidas.Contains(codigoTurma))
            {
                ...
                var validaMatricula = !MatriculaIgualDataConclusaoAlunoTurma(alunoNaTurma);
            ...
            Turma turma, IEnumerable<NotaConceitoBimestreComponenteDto> notasConselhoClasseAluno, IEnumerable<NotaConceitoBimestreComponenteDto> notasFechamentoAluno, bool turmaPossuiRegistroFrequencia, bool componenteLancaNota, bool percentualFrequenciaPadrao, bool visualizaNota, string codigoAluno)
        {
            ...
            var conselhoClasseComponente = new ConselhoClasseComponenteFrequenciaDto()
            {
                ...
                NotaPosConselho = await ObterNotasPosConselho(componenteCurricularCodigo, periodoEscolar?.Bimestre, notasConselhoClasseAluno, notasFechamentoAluno, componenteLancaNota, visualizaNota),
