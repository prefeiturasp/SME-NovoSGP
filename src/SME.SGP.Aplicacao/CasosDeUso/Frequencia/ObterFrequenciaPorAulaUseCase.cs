...
namespace SME.SGP.Aplicacao
{
    public class ObterFrequenciaPorAulaUseCase : AbstractUseCase, IObterFrequenciaPorAulaUseCase
    {
        ...
        public async Task<FrequenciaDto> Executar(FiltroFrequenciaDto param)
        {
            ...
            foreach (var aluno in alunosDaTurmaFiltrados.Where(a => a.EstaAtivo(aula.DataAula, aula.DataAula) || (!a.Inativo && !a.SituacaoMatricula.Equals(SituacaoMatriculaAluno.VinculoIndevido)) ||
            (a.Inativo && a.DataSituacao >= aula.DataAula)).OrderBy(c => c.NomeAluno))
            {
                ...
                var registroFrequenciaAluno = new RegistroFrequenciaAlunoDto
                {
                    ...
                    Desabilitado = (aluno.EstaInativo(aula.DataAula, aula.DataAula) || (aluno.CodigoSituacaoMatricula.Equals(SituacaoMatriculaAluno.Concluido) && aula.EhDataSelecionadaFutura && !aula.PermiteRegistroFrequencia(turma))) && !inativoComDataSituacao,
