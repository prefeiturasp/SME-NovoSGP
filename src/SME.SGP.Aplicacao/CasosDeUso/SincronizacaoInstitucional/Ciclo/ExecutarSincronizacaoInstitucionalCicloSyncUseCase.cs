    ...
    public class ExecutarSincronizacaoInstitucionalCicloSyncUseCase : AbstractUseCase, IExecutarSincronizacaoInstitucionalCicloSyncUseCase
    {
        ...
        public async Task<bool> Executar(MensagemRabbit param)
        {
            ...
            foreach (var ciclo in ciclos)
            {
                try
                {
                    var publicarTratamentoCiclo = await mediator.Send(new PublicarFilaSgpCommand(RotasRabbit.SincronizaEstruturaInstitucionalCicloTratar, ciclo, Guid.NewGuid(), null, fila: RotasRabbit.SincronizaEstruturaInstitucionalCicloTratar));
