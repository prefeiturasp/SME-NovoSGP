...
namespace SME.SGP.Aplicacao
{
    public class ObterDetalhamentoFechamentoConselhoClasseAlunoUseCase : AbstractUseCase, IObterDetalhamentoFechamentoConselhoClasseAlunoUseCase
    {
        ...
        public async Task<IEnumerable<DetalhamentoComponentesCurricularesAlunoDto>> Executar(FiltroConselhoClasseConsolidadoDto param)
        {
            var turma = await mediator.Send(new ObterTurmaPorIdQuery(param.TurmaId));
            if (turma == null)
                throw new NegocioException("Turma não encontrada");
            ...
            var aluno = await mediator.Send(new ObterAlunoPorCodigoEAnoQuery(param.AlunoCodigo, turma.AnoLetivo));
            if (aluno == null)
                throw new NegocioException("Aluno não encontrado");

            var notasFechamento = await mediator.Send(new ObterNotasFechamentosPorTurmasCodigosBimestreQuery(new string[] { turma.CodigoTurma }, aluno.CodigoAluno, param.Bimestre));
