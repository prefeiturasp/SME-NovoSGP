    ...
    public class RemoverAtribuicaoPendenciasUsuariosUseCase : AbstractUseCase, IRemoverAtribuicaoPendenciasUsuariosUseCase
    {
        ...
        public async Task<bool> Executar(MensagemRabbit mensagem)
        {
            try
            {
                ...
                var agruparUePerfilCodigo = ObterAgrupamentoUePerfil(pendenciaFuncionarios);

                await ObterFuncionariosCefaisAdmUes(agruparUePerfilCodigo, dicUePerfilCodigoFuncionarios, lstCefais, lstAdmUes);

                foreach (var pendenciaFuncionario in pendenciaFuncionarios)
                {
                    if (pendenciaFuncionario.UeId.HasValue)
                    {
                        ...
                        switch (perfilUsuario)
                        {
                            ...
                            case PerfilUsuario.DIRETOR:
                                ...
                                var perfilAtual = (int)ObterPerfilPorCargo(funcionarioAtual.CargoId);

                                if (perfilAtual != pendenciaFuncionario.PerfilCodigo)
                    ...
                    {
                        await mediator.Send(new SalvarLogViaRabbitCommand($"Erro na remoção de atribuição de Pendência Perfil Usuário.", LogNivel.Negocio, LogContexto.Pendencia, $"Pendência sem UE: {pendenciaFuncionario.PendenciaId}"));
                    }
                }
