...
namespace SME.SGP.Aplicacao
{
    public class RemoverAtribuicaoResponsaveisASPPPorDreUseCase : IRemoverAtribuicaoResponsaveisASPPPorDreUseCase
    {
        ...
        public async Task<bool> Executar(MensagemRabbit param)
        {
                ...
                if (funcionariosASPP.Any())
                {
                    ...
                    if (funcionarios != null && funcionarios.Any())
                    {
                        if (funcionarios.Count() != supervisoresIds.Count())
                            RemoverASPPSemAtribuicao(funcionariosASPP, funcionarios);
                    }
                    return true;
                }
                return false;
            ...
            {
                await _mediator.Send(new SalvarLogViaRabbitCommand("Não foi possível executar a remoção da atribuição de responsavel (Assistente social, Psicopedagogo, Psicologo) por DRE", LogNivel.Critico, LogContexto.RemoverAtribuicaoReponsavel, ex.Message));
                return false;
            }
        }
        private void RemoverASPPSemAtribuicao(IEnumerable<SupervisorEscolasDreDto> supervisoresEscolasDres, IEnumerable<UsuarioEolRetornoDto> supervisoresEol)
        {
