...
namespace SME.SGP.Aplicacao
{
    public class ExecutarSincronizacaoDresConsolidacaoMatriculasUseCase : AbstractUseCase, IExecutarSincronizacaoDresConsolidacaoMatriculasUseCase
    {
        ...
        {
            var dre = mensagem.ObterObjetoMensagem<DreMatriculaDto>();
            var ues = await mediator.Send(new ObterUesCodigosPorDreQuery(dre.Id));
            foreach (var ueCodigo in ues)
            {
                var ueDto = new UeMatriculaDto(ueCodigo);
                try
                {
                    var publicarTratamentoCiclo = await mediator.Send(new PublicarFilaSgpCommand(RotasRabbit.SincronizarUesMatriculasTurmas, ueDto, new Guid(), null, fila: RotasRabbit.SincronizarUesMatriculasTurmas));
