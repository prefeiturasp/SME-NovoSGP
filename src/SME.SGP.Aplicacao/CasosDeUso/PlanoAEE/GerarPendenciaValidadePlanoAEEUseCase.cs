...
namespace SME.SGP.Aplicacao
{
    public class GerarPendenciaValidadePlanoAEEUseCase : AbstractUseCase, IGerarPendenciaValidadePlanoAEEUseCase
    {
        ...
        private async Task GerarPendenciaValidadePlano(PlanoAEE planoEncerrado, DateTime dataFim)
        {
            ...
            var descricao = $"O Plano AEE {estudanteOuCrianca} {planoEncerrado.AlunoNome} ({planoEncerrado.AlunoCodigo}) da turma {turma.NomeComModalidade()} da {ueDre} expirou em {dataFim:dd/MM/yyyy}. <br/><a href='{hostAplicacao}relatorios/aee/plano/editar/{planoEncerrado.Id}'>Clique aqui para acessar o plano.</a> " +
            ...
            await mediator.Send(new GerarPendenciaValidadePlanoAEECommand(planoEncerrado.Id, usuarioId, titulo, descricao));
        }
