...
namespace SME.SGP.Aplicacao
{
    public class PeriodoDeCompensacaoAbertoUseCase : IPeriodoDeCompensacaoAbertoUseCase
    {
        ...
        public async Task<bool> VerificarPeriodoAberto(string turmaCodigo, int bimestre)
        {
            ...
            if (parametroSistema != null && parametroSistema.Ativo && turma.AnoLetivo == DateTimeExtension.HorarioBrasilia().Year)
                return true;

            var turmaEmPeriodoAberto = await mediator.Send(new TurmaEmPeriodoAbertoQuery(turma, DateTimeExtension.HorarioBrasilia(), bimestre, true));

            return turmaEmPeriodoAberto;
        }
    }
}