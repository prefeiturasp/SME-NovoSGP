    ...
    public class SalvarCompensacaoAusenciaUseCase : AbstractUseCase, ISalvarCompensacaoAusenciaUseCase
    {
        ...
        public async Task Executar(long id, CompensacaoAusenciaDto compensacaoDto)
        {
            try
            {
                ...
                {
                    await GravarDadosCacheParaCalculoFrequencia(turma.CodigoTurma, periodo.Bimestre, codigosComponentesConsiderados.ToArray(), codigosAlunosCompensacao.ToArray(), compensacao.Id, compensacaoAusenciaAlunos.Where(c => !c.Excluido));
