...
public class ObterMensagemNotificacaoAlteracaoParecerConclusivoQueryHandler : NotificacaoParecerConclusivoConselhoClasseCommandBase<ObterMensagemNotificacaoAlteracaoParecerConclusivoQuery, string>
{
    public ObterMensagemNotificacaoAlteracaoParecerConclusivoQueryHandler(IMediator mediator) : base(mediator)
    {
    }
    
    public override async Task<string> Handle(ObterMensagemNotificacaoAlteracaoParecerConclusivoQuery request, CancellationToken cancellationToken)
    {
        ...
        var pareceresConclusivos = (await mediator.Send(new ObterPareceresConclusivosDtoEmAprovacaoPorWorkflowQuery(request.WorkflowAprovacaoId), cancellationToken)).ToList();
