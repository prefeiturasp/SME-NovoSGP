...
namespace SME.SGP.Aplicacao
{
    ...
    public class ObterIdsPendenciaDevolutivaPorAnoLetivoQueryValidator : AbstractValidator<ObterIdsPendenciaDevolutivaPorAnoLetivoQuery>
    {
        public ObterIdsPendenciaDevolutivaPorAnoLetivoQueryValidator()
        {
            ...
            .NotEmpty()
            .WithMessage("O ano letivo deve ser informado para realizar a consulta de pendências de devolutivas.");

            RuleFor(c => c.CodigoUe)
            .NotEmpty()
            .WithMessage("O código da ue deve ser informado para realizar a consulta de pendências de devolutivas" +
                         ".");
        }
    }
}
