    ...
    public class ObterAlunoPorCodigoEolQueryHandler : IRequestHandler<ObterAlunoPorCodigoEolQuery, AlunoPorTurmaResposta>
    {
        ...
        public async Task<AlunoPorTurmaResposta> Handle(ObterAlunoPorCodigoEolQuery request, CancellationToken cancellationToken)
        {
            if (string.IsNullOrEmpty(request.CodigoTurma))
                return (await servicoEol.ObterDadosAluno(request.CodigoAluno, request.AnoLetivo, request.Historico)).OrderByDescending(a => a.DataSituacao)?.FirstOrDefault();
            else
            {
                var retorno = (await servicoEol.ObterDadosAluno(request.CodigoAluno, request.AnoLetivo, request.Historico)).OrderByDescending(a => a.DataSituacao);
