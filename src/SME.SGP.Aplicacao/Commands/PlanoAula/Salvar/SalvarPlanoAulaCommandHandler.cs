        ...
        public async Task<PlanoAulaDto> Handle(SalvarPlanoAulaCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (request.PlanoAula.ComponenteCurricularId.HasValue)
                {
                    ...
                    var componentesCurriculares = await mediator.Send(new ObterComponentesCurricularesPorIdsEUsuariosQuery(new long[] { componenteCurricularId }, codigoTurma: turma.CodigoTurma));
