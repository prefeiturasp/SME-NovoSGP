...
namespace SME.SGP.Aplicacao
{
    public class GerarRelatorioCommandHandler : IRequestHandler<GerarRelatorioCommand, bool>
    {
        ...
        public async Task<bool> Handle(GerarRelatorioCommand request, CancellationToken cancellationToken)
        {
            ...
            await mediator.Send(new PublicaFilaWorkerServidorRelatoriosCommand(request.RotaRelatorio, request.Filtros, request.TipoRelatorio.Name(),
                correlacao.Codigo, request.UsuarioLogadoRf, false, request.PerfilUsuario));
