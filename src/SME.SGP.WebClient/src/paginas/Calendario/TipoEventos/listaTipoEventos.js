...
const ListaTipoEventos = () => {
  ...
  const clicouBotaoNovo = () => {
    history.push('/calendario/tipo-eventos');
  };
  ...
  const [tipoEventosLista, setTipoEventosLista] = useState([]);
  ...
  const aoSelecionarLinha = row => {
    setLinhasSelecionadas(row);
  };
