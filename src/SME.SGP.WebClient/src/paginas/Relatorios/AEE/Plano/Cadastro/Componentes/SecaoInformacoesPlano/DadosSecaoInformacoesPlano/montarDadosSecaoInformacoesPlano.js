...
import { setPlanoAEEEmEdicao } from '~/redux/modulos/planoAEE/actions';
import { erros, setBreadcrumbManual } from '~/servicos';
import ServicoPlanoAEE from '~/servicos/Paginas/Relatorios/AEE/ServicoPlanoAEE';

const MontarDadosSecaoInformacoesPlano = props => {
  const dispatch = useDispatch();

  const { dados, match, codigoAluno, codigoTurma } = props;

  const dadosPlanoAEE = useSelector(store => store.planoAEE.dadosPlanoAEE);
  ...
  const obterQuestionario = useCallback(async questionarioId => {
    const planoId = match?.params?.id;
    dispatch(setPlanoAEEEmEdicao(false));
  ...
  useEffect(() => {
    if (refForm) {
      ServicoPlanoAEE.addFormsSecoesEncaminhamentoAEE(
        () => obterForm(),
  ...
  const onChangeCamposComOpcaoResposta = (
    ...
    dispatch(setPlanoAEEEmEdicao(true));
  };
  ...
  const validaSeDesabilitarCampo = () => {
    ...
    return (
      ...
      (encaminhamentoId && !dadosPlanoAEE.podeEditar) ||
      dadosPlanoAEE?.situacao === situacaoPlanoAEE.Encerrado ||
      dadosPlanoAEE?.situacao === situacaoPlanoAEE.Cancelado
    );
  };
  ...
  return dados?.questionarioId &&
    ...
    <Formik
      ...
      validationSchema={() =>
        ServicoPlanoAEE.obterValidationSchema(dadosQuestionarioAtual, refForm)
      }
      validateOnChange
      validateOnBlur
...
MontarDadosSecaoInformacoesPlano.propTypes = {
...
MontarDadosSecaoInformacoesPlano.defaultProps = {
  dados: {},
  match: {},
  codigoAluno: '',
  codigoTurma: '',
...
export default MontarDadosSecaoInformacoesPlano;
