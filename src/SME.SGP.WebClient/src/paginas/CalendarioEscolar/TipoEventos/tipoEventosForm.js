...
const TipoEventosForm = () => {
  ...
  const clicouBotaoCadastrar = () => {
    setDadosForm({
      descricao: nomeTipoEvento,
      localOcorrencia: localOcorrenciaSelecionado,
      letivo: letivoSelecionado,
      concomitancia,
      tipoData,
      dependencia,
      situacao,
  ...
  useEffect(() => {
    if (Object.entries(dadosForm).length > 0)
      api.post('v1/evento/tipo', dadosForm).then(resposta => {
        console.log(resposta);
      });
  }, [dadosForm]);

  const aoTrocarNomeTipoEvento = () => {
    setNomeTipoEvento(campoNomeTipoEventoRef.current.value);
  };

  useEffect(() => {
    campoNomeTipoEventoRef.current.focus();
    setNomeTipoEvento(nomeTipoEvento);
  }, [nomeTipoEvento]);
          ...
          <Div className="row mb-4">
            <Div className="col-6">
              ...
              <CampoTexto
                ...
                onChange={aoTrocarNomeTipoEvento}
