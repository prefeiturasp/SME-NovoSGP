...
namespace SME.SGP.Dados.Repositorios
{
    public class RepositorioPendenciaAulaConsulta : IRepositorioPendenciaAulaConsulta
    {
        ...
        public async Task<IEnumerable<PendenciaAulaDto>> ObterPendenciasAulasPorPendencia(long pendenciaId)
        {
            try
            {
                var query = @"select distinct a.data_aula as DataAula, pa.Motivo, (a.tipo_aula = @tipoAulaReposicao) ehReposicao, a.turma_id TurmaId, a.ue_id UeId, a.disciplina_id DisciplinaId, 
                           aa.nome_avaliacao as TituloAvaliacao,pe.bimestre,coalesce(cc.descricao_infantil , cc.descricao_sgp, cc.descricao) as ComponenteCurricular,
                           pa.pendencia_id as PendenciaId, t.modalidade_codigo ModalidadeCodigo, t.nome NomeTurma
                           from pendencia_aula pa
                           join pendencia p on p.id = pa.pendencia_id
                           join aula a on a.id = pa.aula_id
                           join componente_curricular cc on cc.id = a.disciplina_id::int4
