    ...
    public class RepositorioDiarioBordo : RepositorioBase<DiarioBordo>, IRepositorioDiarioBordo
    {
        ...
        public async Task<PaginacaoResultadoDto<DiarioBordoResumoDto>> ObterListagemDiarioBordoPorPeriodoPaginado(long turmaId, string componenteCurricularPaiCodigo, long componenteCurricularFilhoCodigo, DateTime? periodoInicio, DateTime? periodoFim, Paginacao paginacao)
        {
            ...
            condicao.AppendLine(@"union all
                           select null id, a.data_aula DataAula, null CodigoRf, null Nome, true Pendente 
                         from aula a
                         inner join turma t on a.turma_id = t.turma_id
                         where t.id = @turmaId
            ...
            var query = $"select count(0) from (select db.id, a.data_aula DataAula, db.criado_rf CodigoRf, db.criado_por Nome, false Pendente {condicao}) as DiarioBordo";
            ...
            query = $"select db.id, a.data_aula DataAula, db.criado_rf CodigoRf, db.criado_por Nome, false Pendente {condicao} order by dataaula desc {offSet} ";
