...
{
    public class RepositorioPainelEducacionalAprovacaoUe
        : IRepositorioPainelEducacionalAprovacaoUe
    {
        private readonly IConfiguration _configuration;
        private readonly ISgpContext _database;

        public RepositorioPainelEducacionalAprovacaoUe(
        ...
        public async Task BulkInsertAsync(
            IEnumerable<PainelEducacionalConsolidacaoAprovacaoUe> indicadores)
        {
            ...
            await using var writer = await conn.BeginBinaryImportAsync(@"
                COPY painel_educacional_consolidacao_aprovacao_ue
                    (codigo_dre, codigo_ue, turma, modalidade_codigo, modalidade,
                     total_promocoes, total_retencoes_ausencias, total_retencoes_notas,
                     ano_letivo, criado_em)
                FROM STDIN (FORMAT BINARY)");

            foreach (var item in indicadores)
                await WriteAprovacaoRowAsync(writer, item);

            await writer.CompleteAsync();
        }

        private static async Task WriteAprovacaoRowAsync(
            NpgsqlBinaryImporter writer,
            PainelEducacionalConsolidacaoAprovacaoUe item)
        {
        ...
        {
            const string sql = @"TRUNCATE painel_educacional_consolidacao_aprovacao_ue";
            await _database.ExecuteAsync(sql);
        }

        
        public async Task<PaginacaoResultadoDto<PainelEducacionalAprovacaoUeRegistroDto>>
            ObterAprovacao(FiltroAprovacaoUeDto filtro)
        {
            ...
            using var multi = await _database.QueryMultipleAsync(query, parametros);
            retorno.Items = multi.Read<PainelEducacionalAprovacaoUeRegistroDto>();
            retorno.TotalRegistros = paginacao.QuantidadeRegistros <= 1
                ? 1
                : multi.ReadFirst<int>();
        ...
        private static void MontarQueryConsulta(StringBuilder sql, Paginacao paginacao, bool contador)
        {
            ...
            sql.AppendLine(
                " WHERE ano_letivo = @anoLetivo AND modalidade_codigo = @modalidadeId AND codigo_ue = @codigoUe ");
