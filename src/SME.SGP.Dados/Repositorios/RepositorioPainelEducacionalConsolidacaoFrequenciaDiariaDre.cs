...
{
    public class RepositorioPainelEducacionalConsolidacaoFrequenciaDiariaDre : IRepositorioPainelEducacionalConsolidacaoFrequenciaDiariaDre
    {
        private readonly IConfiguration configuration;
        private readonly ISgpContext database;

        public RepositorioPainelEducacionalConsolidacaoFrequenciaDiariaDre(IConfiguration configuration, ISgpContext database)
        {
            this.configuration = configuration;
            this.database = database;
        }

        public async Task BulkInsertAsync(IEnumerable<PainelEducacionalConsolidacaoFrequenciaDiariaDre> indicadores)
        {
            await using var conn = new NpgsqlConnection(configuration.GetConnectionString("SGP_Postgres"));
            await conn.OpenAsync();

            await using var writer = conn.BeginBinaryImport(@"
                COPY painel_educacional_consolidacao_frequencia_diaria 
                    (codigo_dre, codigo_ue, nivel_frequencia, ue, ano_letivo, total_estudantes, total_presentes, percentual_frequencia, data_aula, criado_em)
        ...
        {
            var sql = @"TRUNCATE painel_educacional_consolidacao_frequencia_diaria";
