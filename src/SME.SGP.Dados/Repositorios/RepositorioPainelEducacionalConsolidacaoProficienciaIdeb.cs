...
namespace SME.SGP.Dados.Repositorios
{
    ...
    public class RepositorioPainelEducacionalConsolidacaoProficienciaIdeb : IRepositorioPainelEducacionalConsolidacaoProficienciaIdeb
    {
        private readonly IConfiguration configuration;
        private readonly ISgpContext database;
        public RepositorioPainelEducacionalConsolidacaoProficienciaIdeb(IConfiguration configuration, ISgpContext database)
        {
        ...
        {
            const string query = @"delete from painel_educacional_consolidacao_proficiencia_ideb_ue where ano_letivo = @anoLetivo;
                                 -- sincronizar a sequence da tabela
                                 SELECT setval(
                                     pg_get_serial_sequence('painel_educacional_consolidacao_proficiencia_ideb_ue', 'id'),
                                     (SELECT MAX(id) FROM painel_educacional_consolidacao_proficiencia_ideb_ue)
                                 );";
            await database.ExecuteAsync(query, new { anoLetivo });
        }

        public async Task SalvarConsolidacaoAsync(IList<PainelEducacionalConsolidacaoProficienciaIdebUe> indicadores)
        {
            const string comandoCopy = @"
                COPY painel_educacional_consolidacao_proficiencia_ideb_ue 
