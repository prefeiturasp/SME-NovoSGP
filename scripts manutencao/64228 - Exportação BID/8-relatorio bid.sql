...
select t.turma_codigo, t.turma_nome, t.turma_ano, t.ano_letivo
	...
	, max(coalesce(rb.nota_conselho, rb.nota_fechamento)) filter (where rb.componente_curricular_id = 138) as "Nota Lingua Portuguesa"
	, sum(total_aulas) as total_aulas
	, sum(total_ausencias) as total_ausencias
	, sum(total_compensacoes) as total_compensacoes
	, (sum(total_aulas) - sum(total_ausencias) - sum(total_compensacoes))::numeric(15,2) / sum(total_aulas) * 100 as frequencia
  from turmas t
