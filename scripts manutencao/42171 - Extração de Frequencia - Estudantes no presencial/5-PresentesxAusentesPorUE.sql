select AbreviacaoDre, TipoEscola, NomeEscola, CodigoEscola, DataAula
	, count(CodigoAluno) filter (where presente) as presentes
	, count(CodigoAluno) filter (where not presente) as ausentes
from (
...
group by AbreviacaoDre, TipoEscola, NomeEscola, CodigoEscola, DataAula