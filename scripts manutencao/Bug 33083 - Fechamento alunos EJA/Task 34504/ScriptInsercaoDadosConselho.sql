...
begin
	...
	loop
		...
		where ccn.conselho_classe_aluno_id = conselho_classe_aluno_id_referencia and
			  ccn.componente_curricular_codigo = alunos_eja.cd_componente_curricular
		...
		if conselho_classe_nota_id_referencia is null then
			insert into conselho_classe_nota (conselho_classe_aluno_id,
											  ...
											  criado_rf,
											  alterado_rf)
			values (conselho_classe_aluno_id_referencia, alunos_eja.cd_componente_curricular, case when alunos_eja.tipo_nota = 1 then 5 else null end, case when alunos_eja.tipo_nota = 2 then 2 else null end, case when alunos_eja.tipo_nota = 1 then '<p>Continuidade dos estudos</p>' else '<p>S</p>' end, false, false, current_date, 'Sistema', null, null, 'Sistema', null);											  
		end if;		
	end loop;
end $$;