...
declare
	...
	periodo_db record;
begin	
	raise notice 'Iniciando validação frenquência geral bimestral';
	raise notice 'Limpando tabela';
	TRUNCATE TABLE public.frequencia_aluno_geral_inconsistencia;
	...
	for turma_db in 
  	...
  	loop
  		...
  		loop
  			INSERT INTO frequencia_aluno_geral_inconsistencia (codigo_aluno, turma_id, bimestre)
			...
			(select rfa.codigo_aluno, pe.bimestre, turma_id, 
			 ...
			 group by rfa.codigo_aluno, pe.bimestre, turma_id, pe.id) diaria 
			 left join
			(select codigo_aluno, turma_id, 
			 ...
			 and turma_id = turma_db.turma_id 
			 group by codigo_aluno, periodo_escolar_id, turma_id) consolidadoGeral 
			 diaria.turma_id = consolidadoGeral.turma_id 
			 and diaria.codigo_aluno = consolidadoGeral.codigo_aluno 
